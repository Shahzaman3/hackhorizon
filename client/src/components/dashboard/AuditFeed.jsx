...
export default function AuditFeed() {
  ...
  const getActionStyles = (action) => {
    switch (action) {
      ...
      default: return 'text-muted-2 bg-surface-2 border-border';
    }
  };
  ...
  return (
    <div className="bg-white border border-border rounded-[2.5rem] p-10 h-full flex flex-col shadow-sm">
      <div className="mb-10">
         <h3 className="font-extrabold text-xl text-text tracking-tight" style={{ fontFamily: 'Plus Jakarta Sans' }}>Security & Audit Distribution</h3>
         <p className="text-sm font-medium text-muted-2 mt-2">Historical immutable records of all system state changes.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-4">
        {logs.length === 0 ? (
          ...
          <div className="relative border-l-2 border-border ml-4 pl-10 space-y-10 py-4">
            {logs.map((log) => (
              <div key={log._id} className="relative group animate-fade-in">
                {/* Timeline junction */}
                <div className="absolute -left-[49px] top-1 w-6 h-6 bg-white border-2 border-border rounded-xl flex items-center justify-center group-hover:border-[#047857] transition-all group-hover:scale-110 shadow-sm z-10">
                   <div className="w-2 h-2 rounded-full bg-[#D1CFC2] group-hover:bg-[#047857]" />
                </div>
                
                <div className="flex flex-col gap-3 bg-surface-2/20 border border-border/60 p-6 rounded-[1.5rem] hover:border-[#047857]/30 hover:bg-white hover:shadow-xl hover:shadow-[#047857]/5 transition-all duration-300">
                  ...
                  <div className="flex flex-wrap items-center gap-4 mt-2 pt-4 border-t border-border/60">
                    ...
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-[10px] font-black text-text bg-white border border-border px-2 py-0.5 rounded-md font-mono">{log.businessGstin || 'INTERNAL_SYS'}</span>
                    </div>
                  </div>
                </div>
              </div>
