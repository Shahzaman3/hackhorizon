...
const NavIcon = ({ type, active }) => {
  const cls = `w-4 h-4 shrink-0 transition-all duration-300 ${active ? 'text-[#047857]' : 'text-muted-2 group-hover:text-[#047857] group-hover:scale-110'}`;
  const icons = {
...
export default function Sidebar({ role: overrideRole, isOpen, onClose }) {
  ...
  return (
    <div className={`w-65 shrink-0 flex flex-col bg-dark border-r border-border z-40 transition-transform duration-300 h-full fixed md:relative inset-y-0 left-0 md:translate-x-0 ${isOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full shadow-none'}`}>
      ...
      <div className="px-6 py-8 flex flex-col items-center">
        {isOpen && (
          <button
            onClick={onClose}
            className="md:hidden absolute top-4 right-4 w-9 h-9 flex items-center justify-center rounded-xl bg-surface-2 text-muted hover:text-[#047857] transition-colors"
          >
        ...
        <div className="mt-6 w-full flex items-center justify-center">
          <div className="inline-flex items-center gap-2 bg-surface-2 border border-border text-[#047857] text-[10px] font-bold px-4 py-1.5 rounded-full uppercase tracking-[0.15em]">
      ...
      <nav className="flex-1 overflow-y-auto custom-scrollbar px-5 py-2 flex flex-col gap-1.5">
        <div className="mb-4 text-[10px] font-bold text-muted-2 uppercase tracking-[0.2em] px-3">Main Navigation</div>
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3.5 px-4 py-3 rounded-2xl text-sm font-bold transition-all duration-300 group ${
                isActive
                  ? 'bg-[#047857] text-dark shadow-lg shadow-[#047857]/20 translate-x-1'
            ...
            >
              ...
              {isActive && (
                 <div className="ml-auto w-1 h-5 bg-dark/40 rounded-full" />
              )}
            </Link>
          );
        })}
      ...
      <div className="p-6 bg-surface-2/50 border-t border-border">
        {user && (
          <div className="flex items-center gap-3 mb-6 bg-white p-3 rounded-2xl border border-border shadow-sm">
            <div className="w-10 h-10 rounded-xl bg-[#047857] shrink-0 flex items-center justify-center text-dark font-bold text-sm shadow-md shadow-[#047857]/20">
              {getInitials(user.name)}
            </div>
            <div className="flex flex-col overflow-hidden">
              <span className="text-sm font-bold text-text truncate leading-tight" style={{ fontFamily: 'Plus Jakarta Sans' }}>{user.name}</span>
              <span className="text-[10px] text-muted-2 truncate font-medium mt-0.5">{user.email}</span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col gap-2.5">
          <button
            ...
            disabled={switching}
            className="w-full bg-white border border-border rounded-xl py-3 text-[11px] font-bold uppercase tracking-widest text-muted hover:bg-[#047857] hover:text-dark hover:border-[#047857] transition-all duration-300 flex justify-center items-center gap-2.5 disabled:opacity-50 shadow-sm"
          >
            {switching ? (
          ...
          <button
            ...
            className="w-full bg-surface/50 hover:bg-red-50 border border-border hover:border-red-100 rounded-xl py-3 text-[11px] font-bold uppercase tracking-widest text-muted-2 hover:text-red-500 transition-all duration-300 flex items-center justify-center gap-2.5"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
