...
export default function Pricing() {
  ...
  const CheckIcon = () => (
    <svg className="w-4 h-4 shrink-0 mt-0.5 text-[#047857]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  const plans = [
    ...
    },
    {
      ...
      monthlyPrice: 'Rs. 1,499',
      yearlyPrice: 'Rs. 1,199',
  ...
  return (
    <section id="pricing" className="bg-dark py-28 px-6 md:px-10 relative overflow-hidden">
      {/* top glow */}
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="text-center mb-16 flex flex-col items-center">
          <h2
            className="text-[clamp(1.8rem,4vw,2.75rem)] font-extrabold text-text leading-tight mb-4"
          ...
          <div className="inline-flex bg-surface rounded-full p-1 border border-border">
            ...
            >
              ...
              <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-md border leading-none transition-colors duration-300 ${
                isYearly 
                  ? 'bg-dark/20 text-dark border-dark/30' 
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative flex flex-col rounded-2xl p-8 border transition-all duration-300 ${
                plan.featured
                  ? 'bg-surface border-[#047857]/40 shadow-2xl shadow-[#047857]/10 md:scale-[1.04] z-10'
                  : 'bg-surface border-border hover:border-border-2'
              }`}
            >
              {plan.featured && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#047857] text-dark text-[10px] font-extrabold px-4 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-[#047857]/30 whitespace-nowrap">
                  Most Popular
                </div>
              )}

              <h3 className="font-bold text-xl text-text mb-1 text-center" style={{ fontFamily: 'Plus Jakarta Sans' }}>
                {plan.name}
              </h3>
              <p className="text-xs text-muted leading-relaxed min-h-9 mb-6 text-center">{plan.desc}</p>

              <div className="flex items-end justify-center gap-1 mb-8">
                <span
                  className="text-[2.75rem] font-extrabold text-text leading-none tracking-tight"
                  style={{ fontFamily: 'Plus Jakarta Sans' }}
                >
              ...
              <Link
                to={user ? dashboardPath : plan.ctaLink}
                className={`w-full text-center block rounded-xl py-3.5 text-sm font-bold transition-all duration-300 mb-8 ${
                  plan.featured
                    ? 'bg-[#047857] text-dark hover:bg-[#065F46] shadow-lg shadow-[#047857]/25 hover:shadow-[#047857]/40'
                    : 'bg-surface-2 text-text border border-border-2 hover:border-[#047857]/30 hover:bg-[#E5E2D9]'
                }`}
                style={{ fontFamily: 'Plus Jakarta Sans' }}
              >
              ...
              <div className="flex flex-col items-center gap-3.5">
                {plan.features.map((feature, i) => (
                  <div key={i} className="flex gap-2 text-sm items-center text-center">
                    <CheckIcon />
                    <span className={plan.featured ? 'text-text' : 'text-muted'}>{feature}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="mt-12 text-center text-xs text-muted-2 font-semibold tracking-wide">
