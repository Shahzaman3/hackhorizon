  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-dark relative overflow-hidden">
      ...
      <div className="w-full max-w-[420px] relative z-10 flex flex-col">
        ...
        <div className="w-full bg-surface/80 backdrop-blur-xl border border-border rounded-[2rem] p-8 shadow-2xl flex flex-col">
          {/* Role Toggle */}
          <div className="flex w-full bg-dark rounded-2xl p-1 mb-8 border border-border">
            {['seller', 'buyer'].map((r) => (
              <button
                key={r}
                type="button"
                ...
                className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all duration-300 capitalize ${
                  activeRole === r
                    ? 'bg-[#047857] text-dark shadow-lg shadow-[#047857]/25'
          ...
          <h2 className="font-extrabold text-2xl text-text mb-1" style={{ fontFamily: 'Plus Jakarta Sans' }}>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-left">
            {fields.map(({ label, id, type, value, set, placeholder, mono }) => (
              <div key={id}>
                <label className="block text-[11px] font-bold text-muted uppercase tracking-[0.12em] mb-2">
                  {label}
                </label>
                <input
                  type={type}
                  ...
                  className={`w-full bg-surface border border-border rounded-xl px-4 py-3.5 text-sm text-text placeholder-muted-2 outline-none focus:border-[#047857]/50 focus:ring-2 focus:ring-[#047857]/10 transition-all ${mono ? 'font-mono tracking-wider' : ''}`}
                />
              </div>
            ))}
            ...
            {/* Password */}
            <div>
              <label className="block text-[11px] font-bold text-muted uppercase tracking-[0.12em] mb-2">Password</label>
              <div className="relative">
                <input
                  ...
                  placeholder="********"
                  className="w-full bg-surface border border-border rounded-xl px-4 pr-12 py-3.5 text-sm text-text placeholder-muted-2 outline-none focus:border-[#047857]/50 focus:ring-2 focus:ring-[#047857]/10 transition-all tracking-widest"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-2 hover:text-muted text-xs font-bold uppercase tracking-wider transition-colors"
                >
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#047857] hover:bg-[#065F46] text-dark rounded-xl py-4 font-bold text-sm mt-1 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-60 shadow-xl shadow-[#047857]/20 hover:shadow-[#047857]/35"
              style={{ fontFamily: 'Plus Jakarta Sans' }}
            >
              {isLoading ? (
                <>
                  <span className="w-4 h-4 border-2 border-dark/80 border-t-transparent rounded-full spin" />
                  Creating account...
                </>
              ) : (
                'Create account'
          ...
          <button
            ...
            className="w-full bg-surface border border-border hover:border-border-2 hover:bg-surface-2 text-text rounded-xl py-3.5 font-semibold text-sm transition-all flex items-center justify-center gap-3 disabled:opacity-60"
          >
          ...
          <p className="text-center text-sm text-muted mt-8">
            ...
            <Link to="/login" className="text-[#047857] font-bold hover:text-text transition-colors">
              Sign in
            </Link>
          </p>
        </div>
