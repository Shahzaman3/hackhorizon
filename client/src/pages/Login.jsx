      ...
      <div className="w-full max-w-105 relative z-10 flex flex-col items-center">
        ...
        <div className="w-full bg-surface/80 backdrop-blur-xl border border-border rounded-4xl p-8 shadow-2xl flex flex-col text-center">
          <button
            type="button"
            onClick={handleBack}
            className="self-start mb-6 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-muted hover:text-text transition-colors"
          >
          ...
          {/* Role Toggle */}
          <div className="flex w-full bg-surface rounded-2xl p-1 mb-8 border border-border">
            {['seller', 'buyer'].map((r) => (
              <button
                key={r}
                type="button"
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5 text-left">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-surface border border-border rounded-xl px-4 py-3.5 text-sm text-text placeholder-muted-2 outline-none focus:border-[#047857]/50 focus:ring-2 focus:ring-[#047857]/10 transition-all"
              />
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-[11px] font-bold text-muted uppercase tracking-[0.12em]">Password</label>
                <Link to="/forgot-password" className="text-xs font-semibold text-[#047857] hover:text-text transition-colors">
                  Forgot password?
                </Link>
              </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#047857] hover:bg-[#065F46] text-dark rounded-xl py-4 font-bold text-sm mt-1 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed shadow-xl shadow-[#047857]/20 hover:shadow-[#047857]/35"
              style={{ fontFamily: 'Plus Jakarta Sans' }}
            >
              {isLoading ? (
                <>
          ...
          <p className="text-center text-sm text-muted mt-8">
            ...
            <Link to="/register" className="text-[#047857] font-bold hover:text-text transition-colors">
              Register here
            </Link>
          </p>
        </div>
